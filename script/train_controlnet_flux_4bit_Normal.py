...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the JSON file containing validation prompts and images."
    )
    parser.add_argument(
        "--variant",
        type=str,
...
def main(args):
    ...
    quant_config_4bit = DiffusersBitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_compute_dtype=torch.bfloat16,
        bnb_4bit_use_double_quant=True,
        bnb_4bit_quant_type='fp4', # {'fp4', 'nf4'}
    ...
    with accelerator.main_process_first():
        ...
        train_dataset = train_dataset.map(
            compute_embeddings_fn, batched=True, new_fingerprint=new_fingerprint, batch_size=16
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_controlnet):
                ...
                prompt_ids = batch["prompt_ids"].to(dtype=weight_dtype, device=accelerator.device)
                controlnet_mode = torch.tensor([args.controlnet_mode], device=accelerator.device, dtype=torch.long).unsqueeze(0)
            ...
            progress_bar.set_postfix(
                ...
                epoch=f"{(global_step/args.max_train_steps) * args.num_train_epochs:.2f}",
            )
